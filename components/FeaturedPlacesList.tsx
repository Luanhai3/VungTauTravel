...
interface FeaturedPlacesListProps {
  initialPlaces: Place[];
}

export default function FeaturedPlacesList({ initialPlaces }: FeaturedPlacesListProps) {
  ...
  // Logic phân trang
  const totalPages = Math.ceil(initialPlaces.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentPlaces = initialPlaces.slice(startIndex, startIndex + ITEMS_PER_PAGE);

  if (initialPlaces.length === 0) return null;
